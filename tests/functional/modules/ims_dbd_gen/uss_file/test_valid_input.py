...
def test_valid_uss_file_input_single_src(ansible_zos_module):
    ...
    hosts.all.copy(src=os.path.join(LOCAL_SOURCE_DIR,'/uss_file/data/dbdgen02'), dest=SOURCE, checksum='5dd4785e9f4a7d4c4bc36e15ce3b58223113a680', mode='0777')
...
def test_valid_large_uss_file_input_single_src(ansible_zos_module):
    hosts = ansible_zos_module
    hosts.all.copy(
        src=os.path.join(LOCAL_SOURCE_DIR,'/uss_file/data/WMGAT.dbd'), dest=LARGE_SOURCE,
...
def test_valid_uss_file_input_batch(ansible_zos_module):
    hosts = ansible_zos_module
    hosts.all.copy(
        src=os.path.join(LOCAL_SOURCE_DIR,'/uss_file/data/dbdgen02'), dest=SOURCE,
