...
def test_dbd_gen_list_datasets(ansible_zos_module):
    hosts = ansible_zos_module
    batch_list = [
        {'src': SOURCE, 'location': 'DATA_SET', 'member_list': ['DEDBJN21', 'DEDBJNV2', 'DEDBJN21', 'AUTODB', 'DEDBJN21', 'AUTODB', 'DEDBJN21', 'AUTODB'],'replace': True},
        {'src': SOURCE, 'location': 'DATA_SET', 'member_list': ['DEDBJN21', 'DEDBJNV2']},
        {'src': SOURCE, 'member_list': ['DEDBJN21', 'AUTODB'], 'replace': True}
    ]
    validate_batch(hosts, batch_list, DESTINATION, SYSLIB)   
...
def test_dbd_gen_dataset_prereq(ansible_zos_module):
    ...
    response = hosts.all.ims_dbd_gen(dest=DESTINATION, sys_lib=SYSLIB, src=SOURCE, location="DATA_SET", replace=True, 
                member_list=["DH41SK01", "DBFSAMD1", "DH41SK01", "DBFSAMD2","DBFSAMD3", "HOSPVARD", "DSVNTZ30", "DX41SK01", "DX41SK03", "DX41SK05", "DX41SK06", "DX41SK07", "DX41SK08", "DX41SK09", "DX41SK02", "DX41SK04", "WAREDB", "ORDDB", "DISTDB", "ARTDB", "CUSTDB", "NORDDB", "ORDRDB", "ORDLDB", "ITEMDB", "ITEMDBP", "STCKDB"])
    for result in response.contacted.values():
        print(result)
        print("Changed:", result['changed'])
        assert result['changed'] == True
